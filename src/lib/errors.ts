...
import { keySegmentsOnly, toPathArray, toSegments } from './pathModel.js';
...
export function mapErrors(errors: ValidationIssue[], shape: SchemaShape) {
	...
	for (const error of errors) {
		...
		const segments = toSegments(error.path);
		const isLastSegmentIndex = segments[segments.length - 1]?.kind === 'index';

		const objectError =
			!isLastSegmentIndex &&
			pathExists(shape, toPathArray(keySegmentsOnly(segments)))?.value;
...
export function replaceInvalidDefaults<T extends Record<string, unknown>>(
	...
	function Defaults_traverseAndReplace(defaultPath: Partial<PathData>, traversingErrors = false) {
		...
		if (
			...
			const typePath = toPathArray(keySegmentsOnly(toSegments(currentPath)));
